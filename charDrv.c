...
static ssize_t chrdrv_read(struct file *f,char __user *b,size_t c,loff_t *l)
{
...
static int __init chrdrv_init(void)
{
  ...
  if(0 == register_chrdev_region(dev_num,10,"char device"))
  {
  ...
  cdev_init(pcdev,pfo);
  if(0 == cdev_add(pcdev,dev_num,10))
  {
